...
CHECKSUM_FILE="linux/checksums.sha256"
...
# An associative array would read better and is available here, bash on a
# Linux distribution being 4 or newer; the indexed form is kept because
# macos/scripts/utilities/verify-binaries.sh is the same file for a
# different checksum file and cannot use one, macOS shipping bash 3.2, and
# one shape means a correction to either transfers to the other by reading.
rec_path=()
rec_hash=()
rec_ver=()
# Ordered list of unique linux/* paths, in first-seen order.
upaths=()
...
for path in ${upaths[@]+"${upaths[@]}"}; do
    ...
    # shasum before sha256sum, as shared/utilities/lib.sh does: a
    # distribution is not guaranteed to install Perl's Digest::SHA, and
    # coreutils' sha256sum is what is always there, so both are tried
    # rather than either assumed.
